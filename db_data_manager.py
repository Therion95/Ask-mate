...
def voting_for_up_down(cursor, db_table, given_id, up_or_down):
    ...
    # query = sql.SQL("UPDATE {} SET vote_number {} 1 WHERE id = {}").format(*map(sql.Identifier, (db_table, method, given_id)))
    query = util.query_builder('UPDATE', db_table, update= values[up_or_down], condition=f'id = {given_id}',
