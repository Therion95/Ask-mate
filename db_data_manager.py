...
@db_connection.executor
def get_logged_user_name(cursor):
...
def answer_question(cursor, requested_data, requested_image, db_table, question_id):
    ...
    if path:
        values = [str(v) for v in
                  [util.current_date(), 0, question_id, requested_data['message'], path, get_logged_user_name()]]
    else:
